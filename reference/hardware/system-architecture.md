...
| **Flags / branch** | **574×1 FLG** (Z/C) + CW latched `PC_LOAD_EN` |
...
```text
  IR OPC[4:0] ──► CPLD idx5 FSM ──► cw_data/le/bank ──► CW_LO/CW_HI 574×2
                                  ├──► REG_WE (direct)     ├──► MEM_RD/WR, Y_OE, FLG_WE, PC_LOAD_EN
                                  │                        └──► cin/bctrl/lgc/s0/s1 ──► alu8
...
| Date | Note |
|------|------|
| 2026-07-06 | Tier C CW latch (574×2) in block diagram |
