...
| **조립 상세** | [ALU8 단계별 조립](alu8-assembly-spec.md) (12 DIP, 한국어) |
...
목표 클록 (B3c): **2 MHz** — 주기 **500 ns**; posedge→posedge 조합 예산 **500 ns**.
...
- **INC/DEC (B3a):** `net_b0..7` 을 피연산자로 쓰지 말 것. INC=`cin=1` + `bctrl=0000`; DEC=`bctrl=1111`.
...
| **B3a** | ALU 코어 **12** | 12 |
| **B3b** | +574×1 | 13 |
| **B3c** | +2 MHz OSC, +74HC14 (버퍼) | 15 |

참조: [alu8.md](../../hw/netlist/blocks/alu8.md) · [조립 시방](alu8-assembly-spec.md)

B3a: 574, OSC, 74HC14 **없음**.

### 배선 (순서대로)

1. **전원** — 5 V, GND, 모든 DIP에 디커플링.
2. **ALU 코어** — [`alu8.yaml`](../../hw/netlist/blocks/alu8.yaml) ref 순서:  
   283 → **153 mux2** (B 경로·SUB 반전) → 283 → **153 mux1** → **157_YBP** (sum vs logic → Y).  
   CMP 플래그는 SUB 경로 (`net_y`, `net_c_hi`). 별도 74HC04 BINV **없음**.
3. **피연산자** — DIP ×16: `net_a0..7`, `net_b0..7` (INC/DEC 시 B 무시).
4. **제어** — 치트시트대로 DIP/타이: `cin`, `153_s0/s1`, `net_bctrl0..3`, `net_lgc0..3`.  
   SUB/CMP: `bctrl=0011`, `cin=1`.
...
| 블록 | IC |
|------|-----|
| 클록 | +2 — **2.000 MHz** OSC + 74HC14 버퍼 | |

B1 클록 보드 재사용 가능. **`CLK_SYS` / `net_clk2` → `574 CP`** (푸시버튼 제거).

### 배선 변경

| 항목 | 변경 |
|------|------|
| CP | 버튼 제거 → **`net_clk2`** (`CLK_SYS`) |
| OSC | **2.000 MHz** → 74HC14 → `net_clk2` |
...
| `net_a*` 자극 | DIP |
| 제어 넷 | DIP / 치트시트 타이 |
| `net_clk2` @ 2 MHz | 2 MHz OSC + 74HC14 |
| Slack FAIL | 클록 낮추기 또는 SUB 캐리 배선 단축 |

타이밍 (max corner): INC **153 ns**, SUB **136 ns**, logic **46 ns**, ADD **108 ns** (2 MHz 전주기 **500 ns**). [alu-opcodes-timing.md](../hardware/alu-opcodes-timing.md) v1.5.
...
| 증상 | 조치 |
|------|------|
| Y 틀림 (B3a) | 치트시트 재확인; INC=`cin`+`bctrl=0000`, DEC=`bctrl=1111`; 산술은 **157_YBP** |
| CP 후 Q≠Y | Setup 위반 — CP 에지 느리게 또는 Y 안정 후 펄스 |
| 2 MHz에서 Q 틀림 | 스코프 Y vs clk; 배선 단축 또는 클록 하향 |
| pre-flight sim FAIL | `--report` 옵션; B3c는 스코프 |

Netlists are frozen in normative docs ([alu-opcodes-timing.md](../hardware/alu-opcodes-timing.md)); do not regen from external tools in the Active tree.
