...
ABS = HDL / "system_ctrl_gen.abs"

WINCUPL = REPO / "cpld_fsm" / "tools" / "wincupl-ii" / "Shared"

CSIM = WINCUPL / "csim.exe"

CUPL_DL = WINCUPL / "cupl.dl"
...
def test_cyclesim_inactive_idx5(idx5: int) -> None:
    ...
    assert ctrl.eval_comb(ctx) is False





@pytest.mark.skipif(not CSIM.is_file(), reason="WinCUPL csim.exe not installed")

@pytest.mark.skipif(os.name != "nt", reason="csim is Windows-only")

def test_wincupl_csim_vectors() -> None:

    """Optional csim.exe smoke — ATF1504 .abs load often fails on CLI; skip if so."""

    if not ABS.is_file():

        pytest.skip(f"missing {ABS.name} — run build-wincupl.ps1 first")

    subprocess.run([sys.executable, str(GEN_SI)], check=True, cwd=REPO)

    work = Path("C:/plover_csim")
    ...
    shutil.copy2(ABS, work / "system_ctrl.abs")

    shutil.copy2(SI, work / "system_ctrl.si")

    shutil.copy2(CUPL_DL, work / "cupl.dl")

    proc = subprocess.run(

        [str(CSIM), "-u", "cupl.dl", "system_ctrl.si"],
    ...
    if "could not open" in out and "&.abs" in out:

        pytest.skip("csim could not load ATF1504 .abs (known WinCUPL CLI limitation)")

    assert "error" not in out.lower() or "0 error" in out.lower(), out
