...
**Active normative:** [memory-map.md](../../hardware/memory-map.md) (v1.0 flat CPU map)  
**Rationale:** [README.md](README.md)

**Related (archived):** [discrete-mmu-spec-v1.1.md](discrete-mmu-spec-v1.1.md)

v1.1 **does not change** the CPU-visible 16-bit map table in [memory-map.md](../../hardware/memory-map.md) Â§1??. It adds **translation and backing** below the decode layer.
...
## 2. Main physical RAM ??2Ã— IS62C256 (unchanged)

| Chip | CPU range (after MMU) |
|------|------------------------|
| **RAM_1** | `PA15=0` ??`$0000??7FFF` |
| **RAM_2** | `PA15=1` ??Â¬Mailbox | `$8000??FFFF` (except `$FF00??FFFB`) |
...
|-------------|------|---------|
| 0 | 64 KiB | PTE table (16Ã—6 bit used minimum) + metadata headroom |
...
```text
VA ?€?€??[Mailbox/ROM bypass?] ?€?€no?€?€??71024 PTE ?€?€??PA ?€?€??138Ã—2 + glue ?€?€??/CE
         ??yes
         ?”â??€??v1.0 decode (no PTE)
```
