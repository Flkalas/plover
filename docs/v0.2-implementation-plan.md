...
|----|------|-----------|------|----------------|
| **0** | B3 + Phase1 | `alu8`, `alu_decode`, `regfile`, `cpu_datapath_p1` | — | **PASS** (21 tests) |
...
- `hw/netlist/blocks/rom_fetch.yaml` (또는 behavioral ROM + 157/245 버퍼)
- `tools/gen_rom_fetch_netlist.py`
- `hw/tests/rom_fetch_word.yaml` — 주소 N → CW 비트 일치
- `cpu_datapath_p2.yaml` = Phase1 + rom_fetch (PC 스텁)

**완료 기준:** 한 주기에 ROM 워드가 `net_alu_op*`, `net_src_reg*`, `net_dst_reg*`, `net_bus_en*`, `net_ctrl*`에 **게이트/버스로만** 연결; Phase1과 **동일 벡터** 재현 (`pack_rom` hex 1워드).
...
**산출:** `imm_load.yaml`, regfile B측 literal MUX 확장.
