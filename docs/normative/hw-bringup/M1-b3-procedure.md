...
- **INC/DEC (B3a):** `net_b0..7` 을 피연산자로 쓰지 말 것. INC=`net_inc_en=1`; DEC=`bctrl=1111`.
...
4. **제어** — 치트시트대로 DIP/타이: `cin`, `153_s0/s1`, `net_bctrl0..3`, `net_inc_en`, `net_lgc0..3`.  
...
| 증상 | 조치 |
|------|------|
| Y 틀림 (B3a) | 치트시트 재확인; INC/DEC는 `inc_en`/`bctrl`; 산술은 **157_YBP** |
