...
**관련 정본:** [system-architecture.md](../hardware/system-architecture.md) (v1.0) · [software-roadmap.md](../software/software-roadmap.md) (S0–S7) · [hw-bringup/README.md](../hw-bringup/README.md) (M1–M5)
...
| 검증 (bring-up) | §10 | [hw-bringup/README.md](../hw-bringup/README.md) |
...
**Opcode → FSM 템플릿 (16종):** ADD/CMP→`ALU_REG`, LDA/LDIO→`MEM_LD`, STA/STIO/STA16→`MEM_ST`, TFR→`XFER`, BEQ→`BEQ`, JMP→`JMP`, HALT→`HALT`.
...
개발자용 사전 검증: [developer/verification-gates.md](../../developer/verification-gates.md).
...
ALU는 CPU의 **순수 조합 데이터패스**입니다. 두 8비트 피연산자 A, B와 제어 신호(`cin`, `net_bctrl0..3`, `lgc`, `y_mux`)를 받아 결과 Y와 carry를 만듭니다. v1.0 SoC에서는 **제어가 CPLD FSM에서 직접** 나가며, 별도 `alu8_decode` SOP 블록은 사용하지 않습니다 (M1 벤치·P1 bypass용으로만 잔존). [control-and-decode.md](../hardware/control-and-decode.md)
...
- **교육:** 학습자가 opcode별 경로(가산기 vs 논리)를 M1 스코프·[b3-opcode.md](../hw-bringup/b3-opcode.md)로 확인
...
| Reserved (비실기) | `0x16–0x1F`, `0x20+` | — | breadboard 미사용 |
...
v1.0은 **CPLD FSM opcode 테이블**로 마이크로 시퀀스를 구동합니다. Flash `$4000` CW 인덱스는 **미사용** (boot/utility ROM만).
...
마일스톤별 검증: [software-roadmap.md](../software/software-roadmap.md). 개발자 회귀 명령: [developer/verification-gates.md](../../developer/verification-gates.md).
