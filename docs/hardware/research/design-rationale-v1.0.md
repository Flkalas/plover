...
| **ISA** | Opcode `[4:0]`; Extended `0x10每0x1F` (TFR `0x10每0x15`); `0x0C` reserved |
...
Prototype breadboard drove every micro-phase from **10-bit control words** in Flash (`$4000+`), with `alu8_decode` expanding `ALU_OP` on the breadboard.
...
| TFR `0x10每0x15` | none 〞 1-byte implied |
...
| Flash A0每A6 | **Not wired** 〞 no burn of 128 Flash rows |
| Extended opcodes | `0x10每0x1F` 〞 TFR `0x10每0x15`; `0x16每0x1F` reserved |
...
| Benefit | Cost |
|---------|------|
| **?11 DIP** vs prototype Flash-CW control path (`alu8_decode` removed) | **IR[4] ↙ CPLD** (+1 control net) |
...
| Extended ISA `0x10每0x1F` in CPLD | 〞 |
