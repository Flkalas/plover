...
**Status:** Normative overview — **v1.0 P12** (IF\|EX pipe CU + R0/MBR); Gi1 idx5 multiphase archived 2026-07-13; rev G archived 2026-07  
**Audience:** 교육 설계자, 학습자, 기여자, 외부 검토자

**관련 정본:** [system-architecture.md](reference/hardware/system-architecture.md) (v1.0 P12) · [cpld-pipe-cu.md](reference/hardware/cpld-pipe-cu.md) (Active CU) · [software-roadmap.md](reference/software/software-roadmap.md) (S0–S7) · [hw-bringup/README.md](reference/hw-bringup/README.md) (M1–M5; partially legacy)
...
**물리 디코드:** IR[4:0] → **CPLD pipe CU** ([cpld-pipe-cu.md](reference/hardware/cpld-pipe-cu.md)); Flash `$4000` CW **미사용** ([microcode-spec.md](reference/hardware/microcode-spec.md)). Gi1 idx5 **22-row** LUT는 archive.

정본: [plover-whitepaper.md](plover-whitepaper.md) §6 → [microcode-spec.md](reference/hardware/microcode-spec.md) · [cpld-pipe-cu.md](reference/hardware/cpld-pipe-cu.md).

### 6.2 제어: Pipe CU (cw_fsm_only + IF\|EX)

v1.0 P12는 **CPLD pipe FSM**으로 fetch와 execute를 겹칩니다. Flash `$4000` CW 인덱스는 **미사용**. **no idle phases**; lab 실패 시 stretch; ports 실패 시 named FE2 fallback. **no `tfr_valid`**.
...
CPLD bitstream: pipe CU **Design fits pending**. Legacy Gi1 idx5 bitstream은 Active가 아님.
...
Gi1 multiphase (1–3 phase + idle) 실행 모델은 [archive/gi1-v1.0-normative/](archive/gi1-v1.0-normative/)에 보관.
### 6.4 부트·모드
...
| 버전 라벨 | 의미 |
|-----------|------|
| **v1.0 P12** | Normative 하드웨어 (pipe CU + R0/MBR; Gi1 idx5 archived) |
...
| 날짜 | 변경 |
|------|------|
| 2026-07-13 | **v1.0 P12 Active** — pipe CU; Gi1 idx5 → [archive/gi1-v1.0-normative/](archive/gi1-v1.0-normative/) |
