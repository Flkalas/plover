...
**Opcode → FSM:** ADD/CMP→`ALU_REG`, LDA/LDIO→`MEM_LD`, STA/STIO/STA16→`MEM_ST`, BEQ→`BEQ`, JMP→`JMP`, HALT→`HALT`. Frozen idx5 LUT: **20 active rows** ([M3a-control-store.md](reference/hw-bringup/M3a-control-store.md) §2). **`0x10–0x1F` invalid** (no comb TFR).
...
**물리 디코드:** IR[4:0] → CPLD idx5 FSM (**20-row** LUT); Flash `$4000` CW **미사용** ([microcode-spec.md](reference/hardware/microcode-spec.md)).
...
v1.0은 **CPLD FSM opcode 테이블**로 마이크로 시퀀스를 구동합니다. Flash `$4000` CW 인덱스는 **미사용** (boot/utility ROM만). idx5 control store: **20 active rows**; **no `tfr_valid`** (Gi1).

FSM 템플릿: `ALU_REG`, `MEM_LD`, `MEM_ST`, `BEQ`, `JMP`, `HALT`.
