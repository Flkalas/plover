...
| 항목 | 내용 |
|------|------|
| **CPU** | 8-bit TTL datapath: **alu8** (12×74HC DIP) + **ATF1504** 내부 R0–R2 (3fixed) |
...
| **ISA** | Core `0x01–0x0F` + Extended TFR bit-field (`0x11`,`0x12`,`0x14`,`0x16`,`0x18`,`0x19`); `0x0C` reserved |
...
**CPLD 핀·FSM 정본:** [cpld-system-controller.md](reference/hardware/cpld-system-controller.md) §2–3 · **CW latch:** [control-word-latch.md](reference/hardware/control-word-latch.md)
...
| TFR (§6.1) | 없음 (1-byte; dst/src in opcode bit-field) |
| BEQ, JMP | abs16 — MBR / operand latch |

**분기:** CMP/ADD ph0에서 ALU가 **Z/C** 설정 → **574 FLG** → macro_end에서 `PC_LOAD_EN` (BEQ=`FLG_Z`, JMP=무조건).

**CPLD 핀 요약** (Tier C — 상세: [control-word-latch.md](reference/hardware/control-word-latch.md)):

| 방향 | 신호 | 출처 |
|------|------|------|
| In | `OPC[4:0]` | IR[4:0] |
| In | `FLG_Z`, `FLG_C` | FLG 574 |
| Out (direct) | `q_a`, `q_b`, `REG_WE` | CPLD GPR + write strobe |
| Out (direct) | `cw_data[7:0]`, `cw_le`, `cw_bank` | CPLD → CW latch load |
| Out (latched) | `MEM_RD`, `MEM_WR`, `Y_OE`, `FLG_WE`, `PC_LOAD_EN` | CW_LO 574 |
| Out (latched) | `cin`, `bctrl*`, `lgc*`, `s0`, `s1` | CW_LO/CW_HI 574 → ALU |

**Opcode → FSM:** ADD/CMP→`ALU_REG`, LDA/LDIO→`MEM_LD`, STA/STIO/STA16→`MEM_ST`, TFR→comb `tfr_valid` (not idx5 LUT row), BEQ→`BEQ`, JMP→`JMP`, HALT→`HALT`. Frozen idx5 LUT: **20 active rows** ([M3a-control-store.md](reference/hw-bringup/M3a-control-store.md) §2).
...
| Implied TFR | `0x11`,`0x12`,`0x14`,`0x16`,`0x18`,`0x19` | **1 B** | none (`opc[3:2]=dst`, `opc[1:0]=src`) |
| Reserved | `0x10`,`0x13`,`0x15`,`0x17`,`0x1A–0x1F`, `0x20+` | — | breadboard 미사용 |
...
**Implied transfer (TFR bit-field)** — 6502식, 피연산자 없음. `opcode = 0x10 | (dst << 2) | src` (src≠dst, neither field `11₂`):
...
`0x0C` (구 MOV)는 breadboard에서 **reserved**.

**물리 디코드:** IR[4:0] → CPLD idx5 FSM (**20-row** LUT + comb TFR); Flash `$4000` CW **미사용** ([microcode-spec.md](reference/hardware/microcode-spec.md)).
...
v1.0은 **CPLD FSM opcode 테이블**로 마이크로 시퀀스를 구동합니다. Flash `$4000` CW 인덱스는 **미사용** (boot/utility ROM만). idx5 control store: **20 active rows**; TFR uses **`tfr_valid`** comb glue (six opcodes §6.1), not LUT rows.

FSM 템플릿: `ALU_REG`, `MEM_LD`, `MEM_ST`, `XFER` (comb), `BEQ`, `JMP`, `HALT`.
...
- **호출 규약:** S2에서 `CALL`/`RET` + [calling-convention-v0.1.md](reference/software/calling-convention-v0.1.md)
...
START:  LDA $42
        ...
        TFR20          ; R0 → R2 (0x18 implied)
        STA $80
        HALT
```

TFR 니모닉은 어셈블러 확장 대상입니다.
