...
**Architecture:** pure Gigatron **B_CTRL** bit-slice — `U_ALU_153_0..7` (mux1=logic, mux2=`bctrl*`) with operands on shared A/B select.  
**INC:** `cin=1` + `bctrl=0000` → B_add=0 → **A+0+1** (no INC glue).  
...
| mux2 | `2C0..3` = `net_bctrl0..3`, `2Y` = `net_b_add[i]`, `2G` = GND |
| A/B | `A` = `net_a[i]`; `B` = `net_b[i]` (operand select) |
...
| **DEC** | `1111` | constant 1 |
| **INC** | `0000` | B_add=0; use **`cin=1`** on 283 |
...
| `net_cin` | 283 carry in (SUB/CMP/**INC**) |
...
| **ADD / INC** | `net_cin` / `283` → `157_YBP` | **~108** |
